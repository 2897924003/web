<template>
  <q-page class="flex flex-center">
    <div class="q-gutter-md" style="width: 600px;">
      ...
      <!-- 输出卡片 -->
      <q-card v-if="output" class="q-mt-md bg-light-green text-dark q-pa-md" style="border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <q-card-section>
          <strong>输出：</strong>
      ...
      <!-- 错误卡片 -->
      <q-card v-if="error" class="q-mt-md bg-light-red text-dark q-pa-md" style="border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <q-card-section>
          <strong>错误：</strong>
...
const handleRunCode = async () => {
  ...
  try {
    // 第一步：编译代码
    const compileResponse = await gatewayCodeService.post('/compile', code.value, {
      headers: {
