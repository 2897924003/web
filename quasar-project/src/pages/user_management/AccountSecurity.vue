  ...
  <q-page padding>
    ...
    <q-card class="q-mb-md">
      ...
      <q-card-section>
        ...
        <div><strong>账户未过期:</strong> {{ user.accountNonExpired ? '是' : '否' }}</div>
        <div><strong>账户未锁定:</strong> {{ user.accountNonLocked ? '是' : '否' }}</div>
    ...
    <q-card class="q-mb-md">
      ...
      <q-card-section>
        <q-btn label="修改密码" color="primary" @click="showChangePasswordDialog = true" class="q-mb-sm" />
        <q-btn label="更换绑定手机号" color="primary" @click="showChangePhoneDialog = true" class="q-mb-sm" />
        <q-btn label="更换绑定邮箱" color="primary" @click="showChangeEmailDialog = true" class="q-mb-sm" />
        <q-toggle v-model="twoFactorEnabled" label="启用双因素验证" color="primary" @change="toggleTwoFactor" />
      </q-card-section>
    </q-card>
    ...
    <q-dialog v-model="showChangePasswordDialog">
      <q-card style="min-width: 350px;">
        ...
        <q-card-actions align="right">
          <q-btn flat label="关闭" color="negative" @click="showChangePasswordDialog = false" />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
const toggleTwoFactor = async () => {
  try {
    // 假设API返回双因素验证状态
    const response = await api.post('/user/toggle_2fa', { enabled: twoFactorEnabled.value });
